...
// LoginScreen and RegisterScreen removed - using DynamicScreen for auth
...
const TabNavigator = ({ route, navigation: parentNavigation }: any) => {
  ...
  if (screenItems.length === 0) {
    // No tab bar configured for this role - show home screen without tabs
    return (
      <DynamicScreen
        route={{ name: 'Home', params: { screenId: 112, screenName: 'Search Properties' } }}
...
const AppNavigator = () => {
  ...
  // Re-fetch when auth state changes to get role-based home screen
  useEffect(() => {
          ...
          <>
            ...
            >
              {(props) => (
                <DynamicScreen
                  {...props}
                  route={{ ...props.route, params: { screenId: 102, screenName: 'Register' } }}
                />
              )}
            </Stack.Screen>
          </>
